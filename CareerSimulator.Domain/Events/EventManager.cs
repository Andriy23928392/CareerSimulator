...
namespace CareerSimulator.Domain.Events
{
    public static class EventManager
    {
        ...
        public static void TriggerWeeklyEvent(Player player)
        {
            if (_random.Next(1, 101) <= 15)
            {
                Console.Clear();
                Console.ForegroundColor = ConsoleColor.Cyan;
                Console.WriteLine("📸 ПОЗАФУТБОЛЬНЕ ЖИТТЯ");
                Console.WriteLine("Папараці сфотографували вас у нічному клубі перед важливим тренуванням...");
                Console.ResetColor();

                Console.WriteLine("1. Збрехати, що це старе фото (+10 Слави, але є ризик)");
                Console.WriteLine("2. (Відповідально) Вибачитися перед фанатами (+20 Слави за чесність)");
                Console.Write("Ваша вибір: ");

                string choice = Console.ReadLine() ?? "2";
                if (choice == "1" && _random.Next(1, 100) <= 50)
                {
                    Console.WriteLine("\n❌ Вам не повірили. Тренер незадоволений. Слава -20.");
                    player.ChangeReputation(-20);
                }
                else
                {
                    Console.WriteLine("\n✅ Конфлікт вичерпано. Ваша популярність зросла. Слава +20.");
                    player.ChangeReputation(20);
                }
        ...
        public static void TriggerPostMatchInterview(Player player, bool isWin, bool isDraw)
        {
            ...
            {
                Console.WriteLine("Журналіст: 'Результат сьогодні жахливий. Що пішло не так?'");
        ...
        public static void TriggerPreMatchInterview(Player player)
        {
            ...
            Console.WriteLine("\n[Питання 1/3] Журналіст: 'Що ви думаєте про сьогоднішнього суперника? Вони виглядають грізно.'");
            ...
            else if (ans1 == "2")
            {
                ...
                else { player.ChangeReputation(-20); Console.WriteLine("❌ Суперник отримав додаткову мотивацію через вашу самовпевненість. (Слава -20)"); }
            }
            ...
            Console.WriteLine("2. (Впевнено) Який тиск? Я народжений для таких матчів. Сьогодні буде шоу!");
