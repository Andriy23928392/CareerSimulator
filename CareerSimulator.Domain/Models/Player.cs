...
namespace CareerSimulator.Domain.Models
{
    public class Player
    {
        // Зняли всі private set, щоб SaveManager міг записувати сюди дані
        ...
        public int MatchesThisWeek { get; set; } = 0;
        public int TrainingsThisWeek { get; set; } = 0;
        public int Morale { get; set; } = 60;
        public int AgentLevel { get; set; } = 0;

        public int SponsorIncome { get; set; } = 0;
        public string Nationality { get; set; } = "Україна";
        public string SponsorName { get; set; } = "Немає";
        public PlayerStats Stats { get; set; } = new PlayerStats();
        public PlayerUpgrades Upgrades { get; set; } = new PlayerUpgrades();
        public DateTime BirthDate { get; set; }
        public Club? PreContractClub { get; set; } = null;
        ...
        public int CoachTrust { get; set; } = 50;

        public bool IsInjured => WeeksInjured > 0;
        public int WeeksInjured { get; set; } = 0;
        ...
        // ДОДАНО: Порожній конструктор (обов'язкова вимога для завантаження з JSON)
        public Player() { }
        ...
        {
            if (Morale < 30) return (int)(baseCost * 1.5);
            return baseCost;
        }
        ...
        public void SpendEnergy(int baseCost)
        {
            ...
            else throw new Exception($"Недостатньо енергії! (Потрібно: {amount}, є: {Energy})");
        }
        ...
        {
            if (Money - amount < 0) throw new Exception("Недостатньо грошей!");
            Money -= amount;
        }

        public void ChangeRating(int delta)
        {
            Attributes.Pace += delta; Attributes.Shooting += delta; Attributes.Passing += delta;
            Attributes.Dribbling += delta; Attributes.Defending += delta; Attributes.Physical += delta;
            Attributes.GK_Diving += delta; Attributes.GK_Handling += delta; Attributes.GK_Kicking += delta;
            Attributes.GK_Reflexes += delta; Attributes.GK_Speed += delta; Attributes.GK_Positioning += delta;
        }
        ...
        public void LoadState(int age, int energy, decimal money, int rating, string clubName, decimal clubSalary,
                              int villaLvl, int gearLvl, int cryoLvl, int reputation,
                              int totalMatches, int retirementAge)
        {
            ...
            Upgrades.LoadUpgrades(0, villaLvl, gearLvl, cryoLvl, 0);
        }
